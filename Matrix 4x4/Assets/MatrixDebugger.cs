...
public class MatrixDebugger : EditorWindow
{
	...
	private void OnGUI()
	{
		...
		if (target != null)
		{
			...
			EditorGUILayout.LabelField("Translation matrix", style);
			DrawMatrix(MiMatriz4x4.Translate(new Vec3(target.transform.position)));
			
			EditorGUILayout.LabelField("Rotation matrix", style);
			DrawMatrix(MiMatriz4x4.Rotate((Quats)target.transform.rotation));
			
			EditorGUILayout.LabelField("Scale matrix", style);
			DrawMatrix(MiMatriz4x4.Scale(new Vec3(target.transform.position)));
			
			EditorGUILayout.LabelField("TRS matrix", style);
			DrawMatrix(MiMatriz4x4.TRS(new Vec3(target.transform.position), (Quats)target.transform.rotation, new Vec3(target.transform.localScale)));

...
namespace CustomMath {
    ...
    public struct Quats : IEquatable<Quats> {
        ...
        // Resumen:
        //     Returns or sets the euler representation of the rotation.
        ...
        {
            //igual que el vec3 dot expecto que se agrega W
        ...
        public static Quats Euler(Vec3 euler)
        {
            /*
             ...
             s3 = sin(bank / 2) -> x
            */
            float cosY = Mathf.Cos(euler.y * 0.5f); //c1
            float sinY = Mathf.Sin(euler.y * 0.5f); //s1
            float cosZ = Mathf.Cos(euler.z * 0.5f); //c2
            float sinZ = Mathf.Sin(euler.z * 0.5f); //s2
            float cosX = Mathf.Cos(euler.x * 0.5f); //c3
            float sinX = Mathf.Sin(euler.x * 0.5f); //s3
        ...
        public static Quats AngleToRotation(float angle)
        {   // Cos(angulo/2)+Sin(angulo/2)(I,J,K) <- Formula de quaterniones en base a angulos.
            angle *= Mathf.Deg2Rad;
        ...
        public static Quats LookRotation(Vec3 forward, Vec3 upwards) {
            ...
            if (totalSum > 0f)
            {
                ...
                q.w = sqrtTotalSum * 0.5f;
                sqrtTotalSum = 0.5f / sqrtTotalSum;
                q.x = (upwards.z - forward.y) * sqrtTotalSum;
                q.y = (forward.x - right.z) * sqrtTotalSum;
                q.z = (right.y - upwards.x) * sqrtTotalSum;
                return q;
            }
            if ((right.x >= upwards.y) && (right.x >= forward.z))
            {
                ...
                float num4 = 0.5f / num7;
                q.x = 0.5f * num7;
                q.y = (right.y + upwards.x) * num4;
                q.z = (right.z + forward.x) * num4;
                q.w = (upwards.z - forward.y) * num4;
                return q;
            }
            if (upwards.y > forward.z)
            {
                ...
                float num3 = 0.5f / num6;
                q.x = (upwards.x + right.y) * num3;
                q.y = 0.5f * num6;
                q.z = (forward.y + upwards.z) * num3;
                q.w = (forward.x - right.z) * num3;
                return q;
            }
            ...
            float num2 = 0.5f / num5;
            q.x = (forward.x + right.z) * num2;
            q.y = (forward.y + upwards.z) * num2;
            q.z = 0.5f * num5;
            q.w = (right.y - upwards.x) * num2;
            return q;
        }
        // Resumen:
        ...
        public static implicit operator Quaternion(Quats q)
        {

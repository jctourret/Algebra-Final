...
namespace CustomMath {
    ...
    public struct Quats : IEquatable<Quats> {
        ...
        public static implicit operator Quats(Quaternion q)
        {
            return new Quats(q.x, q.y, q.z, q.w);
        }

        public static implicit operator Vec3(Quats q)
        {
        ...
        public static Quats Euler(Vec3 euler)
        {
            ...
            Quats q;
            q.w = (cosX * cosY * cosZ + sinX * sinY * sinZ) * 180.0f / Mathf.PI;
            q.x = (sinX * cosY * cosZ - cosX * sinY * sinZ) * 180.0f / Mathf.PI;
            q.y = (cosX * sinY * cosZ + sinX * cosY * sinZ) * 180.0f / Mathf.PI;
            q.z = (cosX * cosY * sinZ - sinX * sinY * cosZ) * 180.0f / Mathf.PI;
            return q;
        }
        // Resumen:
        //     Returns a rotation that rotates z degrees around the z axis, x degrees around
        ...
        public static Quats LookRotation(Vec3 forward, Vec3 upwards) {
            // Al no saber up se toma inicialmente la default de vec3
            Vec3 up = Vec3.Up;
            // Se normaliza forward para un uso mas comodo?
            forward.Normalize();
            // Se saca right a partir de up y forward y luego se lo normaliza.
            Vec3 upForwardCross = Vec3.Cross(up, forward);
            Vec3 right = new Vec3();

...
namespace CustomMath
{
    public struct Vec3 : IEquatable<Vec3>
    {
        ...
        {
            /*
            Vector projectado sobre onNormal se espresa:
            Vp(vector projectado) = Ps(Projecion escalar de vector sobre onNormal) * (onNormal/Magnitude(onNormal)) (unidad vectorial de OnNormal)

            float scalarVector = Vec3.Dot(vector,onNormal)/Magnitude(onNormal); 

            Vec3 unitVector = onNormal / Magnitude(onNormal);

            Este metodo nos hace hacer Magnitude dos veces, asi que es mejor condensarlo como abajo.
            return scalarVector*unitVector;
            */
            return (Vec3.Dot(vector, onNormal) / Mathf.Pow(Magnitude(onNormal),2)) * onNormal;
        }   
        public static Vec3 Reflect(Vec3 inDirection, Vec3 inNormal)
        {
            Vec3 dirProjectedinNormal = Vec3.Project(inDirection,inNormal);
            return new Vec3(inDirection-(2*dirProjectedinNormal));
        }
